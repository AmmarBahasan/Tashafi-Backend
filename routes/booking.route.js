...
router.post("/new", async (req, res) => {
  try {
    ...
    const sessionID = await Session.findByIdAndUpdate(
      { _id: sID },
      { status: true }
    ).populate({
      path: "doctor",
    ...
    const doctor = await User.findByIdAndUpdate(sessionID.doctor._id, {
      $push: { booked: booking },
    });
    ...
    const patientID = await User.findByIdAndUpdate(
      { _id: sPatient },
...
router.get("/show/:id", async (req, res) => {
  try {
    ...
    //patient
    const patient = await User.findById(user_id).populate({
      path: "booking",
      select: "-password",
      populate: {
        path: "meeting_id",
    ...
    if (doctor && doctor.length) {
      ...
      doctor[0].booked.sort((a, b) => {
        var dateA = new Date(a.meeting_id.start_time),
          dateB = new Date(b.meeting_id.start_time);
        return dateA - dateB;
      });
      ...
      doctor[0].booked.forEach((ele, i) => {
        let done = {
          ...
          meeting: {
            date: convertDate(
              ele.meeting_id.start_time,
              ele.meeting_id.end_time
            ),
      ...
      patient.booking.forEach((ele, i) => {
        let done = {
          ...
          doctor: {
            Fullname:
              ele.meeting_id.doctor.Fname + " " + ele.meeting_id.doctor.Lname,
...
router.post("/finddoctors/", async (req, res) => {
  try {
    ...
    const doctors = await User.find(
      { city: city, specialty: specialty, userType: 1 },
      { password: 0 }
    )
...
router.delete("/:id", async (req, res) => {
  try {
    ...
    const patient = await User.updateMany(
      { booking: oID },
      { $pull: { booking: id } }
    );
    ...
    console.log("--------------------------");
    const doctor = await User.updateMany(
      { booked: oID },
      { $pull: { booked: id } }
    );
...
router.get("/doctorprofile/:id", async (req, res) => {
  try {
    ...
    const doctorr = await User.findById(id)
      .select({ password: 0 })
      .populate("specialty");
    // if the docctor == null or undefind then throw erorr
    ...
    const review = await Review.find({ doctor: id }).populate({
      path: "patient",
      select: "Fname Lname",
    });
    ...
    const sessions = await Session.find({ doctor: id }, { _v: 0 }).populate({
      path: "doctor",
      select: "-password",
      populate: {
        path: "specialty",
